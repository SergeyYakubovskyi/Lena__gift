(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        script.onload = function() {
            ...
            if (typeof particlesJS !== "undefined" && snowElement) particlesJS("snow", {
                particles: {
                    number: {
                        value: 200,
                        density: {
                ...
                retina_detect: true
            }); else console.error('particlesJS не завантажився або елемент з id="snow" не знайдено.');
        };
        script.onerror = function() {
            console.error("Не вдалося завантажити скрипт particles.js");
        };
    ...
    function spawn_snow(snow_density = 200) {
        ...
        if (!snowContainer) {
            console.warn('Елемент з id="snow" не знайдено.');
            return;
        }
    ...
    function spawnSnowCSS(snow_density = 100) {
        ...
        for (let i = 1; i <= snow_density; i++) {
            const random_x = Math.random() * 100;
            const random_offset = random_range(-100, 100) * .01;
            ...
            const random_scale = Math.random();
            const fall_duration = random_range(10, 30);
            const fall_delay = random_int(30) * -1;
            const opacity_ = Math.random();
            rule += `\n                    .${snowflake_name}:nth-child(${i}) {\n                        opacity: ${opacity_};\n                        transform: translate(${random_x}vw, -10px) scale(${random_scale});\n                        animation: fall-${i} ${fall_duration}s ${fall_delay}s linear infinite;\n                    }\n\n                    @keyframes fall-${i} {\n                        ${random_yoyo_time * 100}% {\n                            transform: translate(${random_x_end}vw, ${random_yoyo_y}vh) scale(${random_scale});\n                        }\n                        to {\n                            transform: translate(${random_x_end}vw, 100vh) scale(${random_scale});\n                        }\n                    }\n                `;
        }
        add_css(rule);
    }
    window.addEventListener("load", (() => {
        ...
        } else console.warn('Елемент з id="snow" не знайдено при завантаженні сторінки.');
    }));
    ...
    const outcomes = [ {
        ...
        name: "СНІГ",
        rarity: "legendary",
        image: "https://www.volynnews.com/files/messages/5506/1513602329-4278-shvedskie-uchenyie-opisali-novyiy-vid-snejinki.jpg"
    } ];
